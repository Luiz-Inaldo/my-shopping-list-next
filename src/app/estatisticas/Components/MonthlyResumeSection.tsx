...
export const MonthlyResumeSection = () => {
    ...
    // # React Query
    // ===================
    const { data: purchasesList, isLoading, isFetching, isError, refetch } = useQuery<IPurchaseProps[]>({
      queryKey: [QUERY_KEYS.historic, userProfile?.uid],
      queryFn: () => fetchHistoricData(userProfile?.uid as string),
    ...
    const filterPurchases = async (filter: IFilterProps) => {
        ...
        const filteredData = purchasesList?.filter(purchase =>
            purchase.start_date?.split("T")[0].split("-")[0] === year.toString() &&
            purchase.start_date?.split("T")[0].split("-")[1] === String(month + 1).padStart(2, '0')
        ) || [];
    ...
    useEffect(() => {
        if (purchasesList && purchasesList?.length > 0 && filterLock) {
            setFilterLock(false);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [purchasesList]);
